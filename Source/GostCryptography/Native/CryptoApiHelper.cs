...
namespace GostCryptography.Native
{
	...
	static class CryptoApiHelper
	{
		...
		public static SafeHashHandleImpl CreateHash(SafeProvHandleImpl providerHandle)
		{
			var hashHandle = SafeHashHandleImpl.InvalidHandle;

			if (!CryptoApi.CryptCreateHash(providerHandle, Constants.CALG_GR3411, SafeKeyHandleImpl.InvalidHandle, 0, ref hashHandle))
			{
				throw CreateWin32Error();
			}
		...
		{
			var hashHandle = CreateHash(providerHandle);
